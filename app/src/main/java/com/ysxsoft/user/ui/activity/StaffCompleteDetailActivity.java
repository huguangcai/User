 ...
 * on 2019/12/14 0014
 */
@Route(path = "/main/StaffCompleteDetailActivity")
public class StaffCompleteDetailActivity extends BaseActivity {
    ...
    @BindView(R.id.photoRecycleView1)
    RecyclerView photoRecycleView1;

    @BindView(R.id.tvUpLoad)
    TextView tvUpLoad;
    @BindView(R.id.tvUpLoad1)
    TextView tvUpLoad1;
    @BindView(R.id.tvComplete_time)
    TextView tvComplete_time;

    public static void start(){
        ARouter.getInstance().build(ARouterPath.getStaffCompleteDetailActivity()).navigation();
    }

    @Override
    protected int getLayoutId() {
        return R.layout.staff_complete_detail_layout;
    }
    ...
    private void requestData() {
        OkHttpUtils.get()
                .url(Api.GET_STAFF_COMPLETED_DETAIL)
...
//                Glide.with(mContext).load("").into(riv);
        ...
        BaseQuickAdapter adapter2= new BaseQuickAdapter<String, BaseViewHolder>(R.layout.item_photo_layout, list1) {
            @Override
            protected void convert(BaseViewHolder helper, String item) {
//                RoundImageView riv = helper.getView(R.id.riv);
//                Glide.with(mContext).load("").into(riv);
        ...
        photoRecycleView1.setLayoutManager(new GridLayoutManager(mContext, 3));
        photoRecycleView1.setAdapter(adapter2);
    }
