...
import com.ysxsoft.user.modle.CompleteListDetialResponse;
import com.ysxsoft.user.modle.ServiceListDetialResponse;
 ...
 * on 2019/12/8 0008
 */
@Route(path = "/main/CompletedListDetialActivity")
public class CompletedListDetialActivity extends BaseActivity {
    ...
    public static void start() {
        ARouter.getInstance().build(ARouterPath.getCompletedListDetialActivity()).navigation();
    }
...
//        requestData();
    ...
    private void requestData() {
        OkHttpUtils.post()
                .url(Api.GET_COMPLETE_LIST_DETIAL)
                .addParams("orderId", "")
                .tag(this)
                .build()
                .execute(new StringCallback() {
                    ...
                    public void onResponse(String response, int id) {
                        CompleteListDetialResponse resp = JsonUtils.parseByGson(response, CompleteListDetialResponse.class);
                        if (resp != null) {
                            if (HttpResponse.SUCCESS.equals("0")) {
                                ...
                                tvUpLoad.setText("上传时间：2019-10-26 12:26:25");
                                tvCompleteTime.setText("完成时间：2019-10-26 12:26:25");
                            }
                        }
                    }
                });
...
//                helper.setText(R.id.tvNum,"");
        ...
        adapter1 = new BaseQuickAdapter<String, BaseViewHolder>(R.layout.item_photo_layout, list1) {
            @Override
            protected void convert(BaseViewHolder helper, String item) {
//                RoundImageView riv = helper.getView(R.id.riv);
//                Glide.with(mContext).load("").into(riv);
    ...
    protected int getLayoutId() {
        return R.layout.activity_completed_list_detail_layout;
    }
}
