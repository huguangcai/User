package com.ysxsoft.common_base.permission;
...
import com.ysxsoft.common_base.permission.annotation.OnMPermissionDenied;
import com.ysxsoft.common_base.permission.annotation.OnMPermissionGranted;
import com.ysxsoft.common_base.permission.annotation.OnMPermissionNeverAskAgain;
...
public class MPermission extends BaseMPermission {
    ...
    public static boolean check(Context context, String permission) {
        boolean hashPermission;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {//6.0以上
            if (ActivityCompat.checkSelfPermission(context,permission) == PackageManager.PERMISSION_GRANTED) {//未禁用读取手机号权限
                hashPermission = true;
            } else {//禁用手机号权限
